...
def get_job_config_path(job_name:str) -> Path:
    ...
    home_path.joinpath(f'conf\\{job_name}.yml')
    return home_path.joinpath(f'conf\\{job_name}.yml')
...
def get_files_on_s3(my_bucket: str, my_prefix: str) -> list :
    ...
    try:
        ...
        if 'Contents' in response:
            ...
            # log.debug(contents)
            files = [item['Key'].replace(my_prefix,'') for item in contents if item['Key']!=my_prefix] 
