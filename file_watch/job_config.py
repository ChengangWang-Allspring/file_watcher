...
class JobConfig:
    def __init__(self, job_name: str, job_config_file_path: str) -> None:
        ...
        try:
            ...
            ex.add_note(
                f"Error reading YAML configuration file: {self.JOB_CONFIG_FILE_PATH}"
            )
            raise ex
    ...
    def set_required_group(self, key: str):
        if key not in self.CONFIG_DICT:
            raise JobConfigError(f"Required group key missing in job config: {key}")
        else:
    ...
    def set_required_key(self, key: str, group: str = None) -> None:
        if group:
            if key not in self.CONFIG_DICT[group]:
                raise JobConfigError(
                    f"Required key for group <{group}> missing in job config: {key}"
                )
            else:
                if not self.CONFIG_DICT[group][key]:
                    raise JobConfigError(
                        f"Required value for group <{group}> missing in job config: {key}"
                    )
                else:
                    setattr(self, key, self.CONFIG_DICT[group][key])
        else:
            if key not in self.CONFIG_DICT:
                raise JobConfigError(f"Required key missing in job config: {key}")
            else:
                if not self.CONFIG_DICT[key]:
                    raise JobConfigError(f"Required value missing in job config: {key}")
                else:
                    setattr(self, key, self.CONFIG_DICT[key])
    ...
    def set_environ_overrides(self) -> None:
        if self.force_env and (self.force_env not in self.CONFIG_DICT):
            raise JobConfigError(
                f"Required env override missing in job config: {self.force_env}"
            )
        elif not self.CONFIG_DICT[self.force_env]:
            raise JobConfigError(
                f"Env overide is empty in job config: {self.force_env}"
            )
        else:
            for key in self.CONFIG_DICT[self.force_env]:
                if not hasattr(self, key):
                    raise JobConfigError(
                        f"Not defined attribute in job config: {self.force_env} -> {key}"
                    )
        ...
        if self.copy_file:
            if self.target_s3_uri:
                if self.target_s3_uri == self.source_s3_uri:
                    raise JobConfigError(
                        f"Conflict error: target_s3_uri and source_s3_uri are the same "
                    )
        if self.archive_file:
            if self.archive_s3_uri:
                if (self.archive_s3_uri == self.source_s3_uri) or (
                    self.archive_s3_uri == self.target_s3_uri
                ):
                    raise JobConfigError(
                        f"Conflict error: archive_s3_uri, target_s3_uri, source_s3_uri has to be unique "
                    )
    ...
    def check_permission(self) -> None:
        ...
        if self.copy_file:
            if self.target_s3_uri:
                ...
                if not is_s3_writable(self.copy_s3_bucket, self.copy_s3_prefix):
                    raise JobConfigError(
                        f"target_s3_bucket is not writable: {self.copy_s3_bucket} | {self.copy_s3_prefix}"
                    )
            elif self.target_path:
                if not is_path_writable(self.target_path):
                    raise JobConfigError(
                        f"target_path is not writable: {self.target_path}"
                    )
            else:
                raise JobConfigError(
                    "Missing config: copy_file = True, however target_path is null "
                )
            ...
            if not is_s3_writable(self.archive_s3_bucket, self.archive_s3_prefix):
                raise JobConfigError(
                    f"archive_s3_bucket is not writable: {self.archive_s3_bucket} | {self.archive_s3_prefix}"
                )
            elif self.archive_path:
                if not is_path_writable(self.archive_path):
                    raise JobConfigError(
                        f"archive_path is not writable: {self.archive_path}"
                    )
            else:
                raise JobConfigError(
                    "Missing config: archive_file = true, however archive_path is null"
                )
    ...
    def source_s3_bucket(self):
        if self.is_source_s3:
            [bucket, prefix] = get_s3_bucket_prefix_by_uri(self.source_path)
            return bucket
        else:
            return None
    ...
    def copy_s3_bucket(self):
        if self.is_copy_s3:
            [bucket, prefix] = get_s3_bucket_prefix_by_uri(self.copy_path)
            return bucket
        else:
            return None
    ...
    def archive_s3_bucket(self):
        if self.is_archive_s3:
            [bucket, prefix] = get_s3_bucket_prefix_by_uri(self.archive_path)
            return bucket
        else:
            return None
